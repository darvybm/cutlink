        ...
        <div class="row d-flex">
            ...
            <div class="col mapcol">
                <div class="map-container">
                    <svg id="mapa" viewBox="0 0 950 620"
...
<script>
    ...
    var myChart = new Chart(ctx, {
        type: "bar",
        data: {
            labels: [<#list topURL as url>"${url.urlOriginal}"<#if url_has_next>, </#if></#list>],
            datasets: [{
                label: "Urls con mas accesos",
                data: [<#list cantTopURL as cant>${cant}<#if cant_has_next>, </#if></#list>],
                backgroundColor: [
                    "#00BEC6",
                ],
                borderColor: [
...
<!-- Agrega un contenedor para la etiqueta flotante -->
<div id="info-pais"></div>

<script>
    <#if Paises??>
    var datos = [
        <#list Paises?keys as pais>
        {pais: "${pais}", accesos: ${Paises[pais]}}<#if pais_has_next>,</#if>
        </#list>
    ];
    </#if>

    var paises = document.querySelectorAll("path");
    for (const pais of paises) {
        pais.setAttribute("fill", "#b4d1e0");
        // Agrega el evento mousemove
        pais.addEventListener("mousemove", function(event) {
            // Busca el país correspondiente en los datos
            var paisDatos = datos.find(function(elemento) {
                return (elemento.pais.toLowerCase()) === (event.target.id.toLowerCase());
            });
            // Actualiza el contenido de la etiqueta flotante con el nombre y los accesos del país
            var infoPais = document.getElementById("info-pais");
            infoPais.innerHTML = paisDatos.pais + ": " + paisDatos.accesos + " accesos";
            // Actualiza la posición de la etiqueta flotante para que siga el cursor
            infoPais.style.left = event.pageX + 10 + "px";
            infoPais.style.top = event.pageY + 10 + "px";
            infoPais.style.display = "block";
        });
        // Agrega el evento mouseout
        pais.addEventListener("mouseout", function(event) {
            // Oculta la etiqueta flotante
            var infoPais = document.getElementById("info-pais");
    ...
    for (const pais of datos) {
        var paisElement = document.getElementById((pais.pais.toLowerCase()));
        console.log(pais);
        if (pais.accesos > 10)
            paisElement.setAttribute("fill", "#48697a");
        if (pais.accesos > 30)
            paisElement.setAttribute("fill", "#314a56");
        if (pais.accesos > 60)
            paisElement.setAttribute("fill", "#273a42");
        if (pais.accesos > 90)
            paisElement.setAttribute("fill", "#1b2b31");
        if (pais.accesos > 150)
            paisElement.setAttribute("fill", "#111a1e");
        if (pais.accesos <= 10)
            paisElement.setAttribute("fill", "#85b6c7");
    }

    //Rotar al pais con más accesos
    const svgElement = document.querySelector('svg');

    const newTranslateX = -25;
    const newTranslateY = -35;
    ...
    //Rotar al pais con más accesos
    svgElement.style.transformOrigin = 0 + 'px, '+ 0 + 'px';
    svgElement.style.transition = 'transform 2s';
    svgElement.style.transform = 'translate(' + newTranslateX + '%, ' + newTranslateY + '%)';
</script>
